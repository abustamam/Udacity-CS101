...
def create_data_structure(string_input):
    people = string_input.split('.')
#    connections = [people[x] for x in range(len(people)) if x%2 == 0 and people[x]]
#    games = [people[y] for y in range(len(people)) if y%2 != 0 and people[y]]
    network = {}
    peeps, vidz = [],[]
    for person in people:
        idx = person.find(' ')
        dude = person[:idx]
        jdx = person.find('is connected to ')
        if jdx > 0:
            peeps = person[idx+17:].split(', ')
        kdx = person.find('likes to play ')
        if kdx > 0:
            vidz = person[idx+15:].split(', ')
        if dude:
            network[dude] = [peeps, vidz]
    return network
...
def get_secondary_connections(network, user):
    ...
    if not network[user][0]:
        return []
    conn = network[network[user][0][0]][0]
    for person in network[user][0][:]:
        conn = merge_list(conn,network[person][0])
    return conn
...
def connections_in_common(network, user_A, user_B):
    if user_A not in network or user_B not in network:
        return False
    netA = network[user_A][0]
    netB = network[user_B][0]
    res = 0
    for person in netA:
        if person in netB:
            res += 1
...
def path_to_friend(network, user_A, user_B, visit=None):
    ...
    for person in connections:
        if person not in visit:
            newpath = path_to_friend(network, person, user_B, visit)
